...
<body>
        ...
        <table>
            ...
            if ($consulta->execute()) {
                while ($fila = $consulta->fetch(PDO::FETCH_OBJ)) {
                    ...
                    // echo "
                    // <div id='$id_oferta' class='modal' >
                    // <div class='modal-content'>
                    // <span class='closeI' onclick=\"cerrarModalInscritos('$id_oferta')\">&times;</span>  
    ...
    <div id="miModal" class="modal">
        <div class="modal-content">
            <span class="closeI" id="cerrarModal">&times;</span>
            <h2>Alumnos inscritos</h2>
            <!-- Aquí se agregarán dinámicamente los datos de los alumnos desde JavaScript -->
            <div id="datosAlumnos"></div>
        </div>
    </div>


<!-- Agrega esto en tu documento HTML, preferiblemente antes de cerrar la etiqueta </body> -->
<script>
    document.addEventListener("DOMContentLoaded", function() {
        var cerrarModalButton = document.getElementById('cerrarModal');
        var modal = document.getElementById('miModal');

        // Cierra la modal cuando se hace clic en el botón de cierre
        cerrarModalButton.addEventListener('click', function() {
            modal.style.display = 'none';
        });

        // Función para cargar y mostrar la ventana modal
        function cargarVentanaModal(id_oferta) {
            ...
            fetch('obtener_datos_alumnos.php?id_oferta=' + id_oferta).then(response => response.json()).then(datosAlumnos => {
        ...
        // Función para pintar los datos de los alumnos en la modal
        function pintarDatosEnModal(datosAlumnos) {
            var datosAlumnosDiv = document.getElementById('datosAlumnos');
            datosAlumnosDiv.innerHTML = ''; // Limpiar contenido anterior

            // Itera sobre los datos y crea elementos para cada alumno
        ...
        // Obtén todos los botones con la clase 'btn_inscritos'
        var buttons = document.querySelectorAll('.btn_inscritos');

        // Adjunta un evento de clic a cada botón
        buttons.forEach(function(button) {
            button.addEventListener('click', function() {
                // Obtén el ID de oferta desde el atributo data-modal-id
                var id_oferta = this.getAttribute('data-modal-id');

                // Llama a la función para cargar y mostrar la ventana modal
