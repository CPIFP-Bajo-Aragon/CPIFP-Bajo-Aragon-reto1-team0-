...
function listarOfertasDesdeAlumno($conexion) {
    ...
    $sql = "SELECT ot.*, p.nombre as Nombre_Poblacion, e.nombre_empresa, e.id_sector as id_sector_empresa, s.nombre_sector
    ...
    if ($consulta->execute()) {
        ...
        while ($fila = $consulta->fetch(PDO::FETCH_OBJ)) {
            ...
            echo "<p class='sector' id='" . $fila-> id_sector_empresa . "'><strong>Sector:</strong> " . $fila-> nombre_sector . "</p>";
        ...
        function editarDatosPersonales($conexion)
        {
            ...
            if (isset($_POST['Guardar'])) {
                ...
                $update_alumno_sql = "UPDATE alumno 
                                    SET foto = ?, apellidos = ?, dni = ?, nombre = ?, telefono = ?, fecha_nacim = ?, carnet_conducir = ?, 
                                        actitudes = ?, aptitudes = ?,  id_poblacion = ? 
                                    WHERE id_usuario = ?";
                $update_alumno_stmt = $conexion->prepare($update_alumno_sql);
                $update_alumno_stmt->execute([$imagen_base64, $apellidos, $dni, $nombre, $telefono, $fecha_nacim, $carnet_conducir, $actitudes, $aptitudes, $id_poblacion, $id_usuario]);
