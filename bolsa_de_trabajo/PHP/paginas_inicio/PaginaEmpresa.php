...
<body>  
    ...
    <main id="mainofertaspublicadasempresa">
        ...
        <div class="todasOfertas">
            ...
            <div id="tabla">
                    <?php
                        ...
                        $sql = "SELECT OT.id_oferta as id_oferta, OT.titulo AS Titulo, OT.descripcion_oferta AS Descripcion_Oferta, OT.fecha_publicacion AS Fecha_Publicacion, OT.duracion_contrato AS Duracion_Contrato, OT.carnet_conducir AS Carnet_Conducir, OT.vehiculo_propio, OT.aptitud, activa, P.nombre AS Nombre_Poblacion, P.id_poblacion as id_poblacion FROM oferta_trabajo AS OT JOIN poblacion AS P ON OT.id_poblacion = P.id_poblacion JOIN empresa AS E ON OT.id_usuario = E.id_usuario WHERE E.id_usuario=$id_usuario ORDER BY fecha_publicacion DESC";  //LIMIT $inicio, $max_filas_por_pagina
                            ...
                            while ($fila = $consulta->fetch(PDO::FETCH_OBJ)) {
                                ...
                                $activa = $fila->activa;
                                    ...
                                    echo "<section class='sectionPrueba'>";
                                        ...
                                        echo "<div id='datos1'><p></p><h1 class='titulo-oferta'>" . $fila->Titulo." "."<i class='fas fa-caret-down'></i> "."</h1>";                                
                                        echo "<p class='fecha'>" . date('d-m-Y', strtotime($fila->Fecha_Publicacion)) . "</p></div>";                     
                                        ...
                                        echo "</div>";
                                        // echo "<button type='button' name='editar' id='editar_$fila->id_oferta'><i class='fas fa-pencil-alt'></i></button>";
                                        echo "<div style='display: flex;'>";
                                        echo "<button type='button' name='editar' id='editar_$fila->id_oferta' style='background-color: #a9dfbf; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-right: 5px;'><i class='fas fa-pencil-alt'></i></button>"; 
                                        echo "<form method='POST'>";
                                        if($activa==1){
                                        // echo "<button class='btn_desactivar' name='desactivar' id='desactivar_$id_oferta'>Desactivar</button>";
                                        echo "<button class='btn_desactivar' name='desactivar' id='desactivar_$id_oferta' style='background-color: #ff6347; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px;'>Desactivar</button>";
                                        }else{
                                        // echo "<button class='btn_activar' name='activar' id='activar_$id_oferta'>Activar</button>";
                                        echo "<button class='btn_activar' name='activar' id='activar_$id_oferta' style='display: inline-block; background-color: #4b89da; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px;'>Activar</button>";
                                        }
                                        echo "</div>";
                                        echo "</form>";
                                    echo "</section>";
        ...
        <div id="editaroferta">
            <div class="modal-content-editar" id="classeditar">
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        cerrarModalButton.addEventListener('click', function() {
            ...
            var shouldOpenModalAutomatically = false;  // Cambia a 'false' si no quieres que se abra automáticamente
    ...
    function pintarDatosEnModal(datosAlumnos, id_oferta) {
        ...
        datosAlumnos.forEach(function(alumno) {
            num=0;
            var id_usuario = alumno.id_usuario;
            num+1;
...
document.querySelectorAll('[name="editar"]').forEach(function (button) {
    var modalContent = document.getElementById('classeditar');
    ...
    button.addEventListener('click', function (event) {
        ...
        inputSubmit.id= 'idBotonGuardarEditar';
        ...
        labelDuracion.textContent = 'Duración Contrato:';

        var labelcarnet = document.createElement('label');
        labelcarnet.textContent = '¿Solicita tener Carnet?';
        ...
        cargarVentanaModaleditar(idUsuario);

        openModal('classeditar');
    });
});
...
function cargarVentanaModaleditar(idUsuario) {
