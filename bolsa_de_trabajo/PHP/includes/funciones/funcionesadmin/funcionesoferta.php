        ...
        function listarofertas($conexion, $max_filas_por_pagina){
            ...
            if ($consulta->execute()) {
                while ($fila = $consulta->fetch(PDO::FETCH_OBJ)) {
                    ...
                    echo "<td> <div id='divdescripcion'>" . $fila->Descripcion_Oferta . "</div></td>";
                    ...
                    echo "<td>" . $fila->Nombre_Empresa . "</td><td>";
                            ...
                            echo "<button type='submit' name='validar' id='editar' style='background-color: #add8e6; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-left: 20%;'>validar</button>";
                            
                            }
                            if($fila->activa==1){
                                // echo ("<button type='submit' name='desactivar' id='desactivar'>desactivar</button>");
                                echo "<button type='submit' name='desactivar' id='desactivar' style='background-color: #ff6347; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-left: 20%;'>desactivar</button>";


                            }else{
                                // echo ("<button type='submit' name='activar' id='activar'>activar</button>");
                                echo "<button type='submit' name='activar' id='activar' style='background-color: #add8e6; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-left: 20%;'>activar</button>";

                            }
                            // echo "<button type='submit' name='editar' id='editar_$fila->id_oferta'><i class='fas fa-pencil-alt'></i></button>";
                            echo "<button type='submit' name='editar' id='editar_$fila->id_oferta' style='background-color: #a9dfbf; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-left: 20%;'><i class='fas fa-pencil-alt'></i></button>";
    ...
    //Funcion para insertar una oferta Admin
        function insertarofertasadmin($conexion, $estudios, $experiencia, $idiomas){
            if(isset($_POST['insertoferta'])){
                ...
                $sql="INSERT INTO `oferta_trabajo`(`titulo`, `descripcion_oferta`, `fecha_publicacion`, `duracion_contrato`, `aptitud`, `carnet_conducir`, `id_poblacion`, `id_usuario`, activa, validada, vehiculo_propio) VALUES ('$titulo','$descripcion','$fecha','$duracion','$aptitud','$conducir','$id_poblacion','$id_empresa', 1, 1, $coche)";
