...
<body>
    ...
    if (isset($_POST['Guardar'])) {
        ...
        if ($count == 0) {
            ...
            if ($insert_stmt->rowCount() > 0) {
                header("location: experiencia-laboral-alumno");
    ...
    if (isset($_POST['eliminar'])) {
        // Obtener el ID de la experiencia laboral a eliminar
        $experiencia_id = $_POST['experiencia_id'];
    
        // Verificar si la experiencia laboral pertenece al usuario actual (opcional, pero recomendado)
        $verificacion_sql = "SELECT id_usuario FROM poseer_experiencia WHERE id = ?";
        $verificacion_stmt = $conexion->prepare($verificacion_sql);
        $verificacion_stmt->execute([$experiencia_id]);
        $experiencia_usuario = $verificacion_stmt->fetchColumn();
    
        // Verificar si la experiencia laboral pertenece al usuario actual (opcional, pero recomendado)
        if ($experiencia_usuario == $id_usuario) {
            // Eliminar la experiencia laboral
            $delete_sql = "DELETE FROM poseer_experiencia WHERE id = ?";
            $delete_stmt = $conexion->prepare($delete_sql);
            $delete_stmt->execute([$experiencia_id]);
    
            if ($delete_stmt->rowCount() > 0) {
                echo "Experiencia laboral eliminada con éxito.";
            } else {
                echo "Error al eliminar la experiencia laboral.";
            }
        } else {
            echo "La experiencia laboral no pertenece al usuario actual.";
        }
    }
    ?>
