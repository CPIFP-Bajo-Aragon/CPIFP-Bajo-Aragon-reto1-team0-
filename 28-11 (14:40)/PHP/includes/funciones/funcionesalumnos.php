...
function listarOfertasDesdeAlumno($conexion) {
    ...
    $sql = "SELECT ot.*, p.nombre as Nombre_Poblacion, e.nombre_empresa, e.id_sector as id_sector_empresa, o.id_sector as id_sector_oficio, s.nombre_sector as nombre_sector_empresa, s_oficio.nombre_sector as nombre_sector_oficio, o.puesto_trabajo, est.nombre_estudio, i.nombre as nombre_idioma, n.nivel as nivel_idioma
    ...
    if ($consulta->execute()) {
        ...
        while ($fila = $consulta->fetch(PDO::FETCH_OBJ)) {
            ...
            //idioma requerido
            $nombre_idioma = $fila->nombre_idioma;

            //nivel requerido
            $nivel_idioma = $fila->nivel_idioma;
            ...
            echo "<h2>$titulo</h2>";
            echo "<p class='puesto_trabajo'><strong>Puesto de Trabajo:</strong> " . $fila->puesto_trabajo . "</p>";
            echo "<p class='sector' id='" . $fila-> id_sector_oficio . "'><strong>Sector Puesto trabajo:</strong> " . $fila-> nombre_sector_oficio . "</p>";
            ...
            echo "<p>A PARTIR DE AUI METER EN MODAL</p>";
            echo "<p>$descripcion</p>";

            echo "<p class='duracion-contrato'><strong>Duración del contrato:</strong> $duracion_contrato meses</p>";
            echo "<p class='carnet_conducir'><strong>Carnet Conducir:</strong> $carnet_conducir_texto </p>";
            echo "<p class='vehiculo_propio'><strong>Requiere de vehiculo propio:</strong> $vehiculo_propio_texto </p>";
            echo "<p class='estudios_requeridos'><strong>Estudios requeridos:</strong> $nombre_estudio </p>";
            echo "<p class='idiomas_requeridos'><strong>Idioma requerido: </strong> $nombre_idioma (nivel: $nivel_idioma) </p>";

            echo "<p><strong>Empresa:</strong>" . $fila->nombre_empresa . "</p>";
            echo "<p class='sector' id='" . $fila-> id_sector_empresa . "'><strong>Sector Empresa:</strong> " . $fila-> nombre_sector_empresa . "</p>";
        ...
        function editarDatosPersonales($conexion)
        {
            ...
            if (isset($_POST['Guardar'])) {
                ...
                $update_alumno_sql = "UPDATE alumno 
                                    SET  apellidos = ?, dni = ?, nombre = ?, telefono = ?, fecha_nacim = ?, carnet_conducir = ?, 
                                        actitudes = ?, aptitudes = ?,  id_poblacion = ? 
                                    WHERE id_usuario = ?";
                $update_alumno_stmt = $conexion->prepare($update_alumno_sql);
                $update_alumno_stmt->execute([$apellidos, $dni, $nombre, $telefono, $fecha_nacim, $carnet_conducir, $actitudes, $aptitudes, $id_poblacion, $id_usuario]);
    ...
    if ($consulta->execute()) {
        ...
        while ($fila = $consulta->fetch(PDO::FETCH_OBJ)) {
            ...
            if (!$no_inscrito) {
                ...
                echo '<p class="ya-inscrito"><strong>Inscripcion anulada</strong></p>';
            }
